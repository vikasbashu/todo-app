...
app.post('/todos/users/signup', async (req, res) => {
  ...
  try{
    ...
    if(existing_account) res.status(403).json({message: 'username already exists. Please use another one!'});
    else{
      const new_account = new USERS(req.body);
      await new_account.save();
      res.status(201).json({message: 'User created successfully', token: create_token({username})});
    }
  }catch(err){
...
app.post('/todos', async (req, res)=> {
  ...
  if(! title.length && description.length) res.status(400).json({message: 'Invaid input for title/description'});
  else{
    const new_todo_item = new TODOS({title, description});
